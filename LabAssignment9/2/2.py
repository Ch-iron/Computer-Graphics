...
def exp(rv):
    ...
    uy = u[1]
    uz = u[2]
    return np.array([[(1-cos)+ux**2*cos, ux*uy*cos-uz*sin, ux*uz*cos+uy*sin],
                     [uy*ux*cos+uz*sin, (1-cos)+uy**2*cos, uy*uz*cos-ux*sin],
                     [uz*ux*cos-uy*sin, uz*uy*cos+ux*sin, (1-cos)+uz**2*cos]])
...
def log(R):
    ...
    return np.array([v[0], v[1], v[2]]) * theta

def slerp(R1, R2, t):
    return R1 @ exp(t*log(R1.T @ R2))

def interpolateRotVec(rv1, rv2, t):
    return lerp(exp(rv1), exp(rv2), t)
